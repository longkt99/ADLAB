// ============================================
// AdLab Attestation API
// ============================================
// PHASE D32: External Attestation Mode.
//
// PROVIDES:
// - GET: Generate attestation for specified profile
//
// INVARIANTS:
// - READ-ONLY endpoint
// - Owner/Admin only
...
// - HTTP 412 if required evidence missing
// ============================================
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Generate attestation with redactions
    ...
    if (attestation.overallStatus === 'FAIL' && attestation.summary.criticalFailures > 0) {
      ...
      await appendAuditLog({
        ...
        action: 'VALIDATE',
        entityType: 'dataset',
        entityId: 'attestation',
        scope: {
          platform: 'system',
          dataset: 'attestation',
        },
        metadata: {
          attestationAction: 'ATTESTATION_FAILED',
          profile: profileId,
      ...
      return NextResponse.json(
        {
          ...
          attestation,
          failedInvariants: attestation.invariantResults.filter(
            (i) => i.status === 'FAIL' && i.critical
          ),
        },
        { status: 412 }
      );
    ...
    await appendAuditLog({
      ...
      metadata: {
        attestationAction: 'ATTESTATION_VIEWED',
        profile: profileId,
        overallStatus: attestation.overallStatus,
    ...
    console.error('D32: Attestation error:', e);
    return NextResponse.json(
